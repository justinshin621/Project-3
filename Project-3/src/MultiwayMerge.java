        ...
        else
        {
            ...
            do
            {
                    ...
                    for (int i = 0; i < runList2.size(); i += 16)
                    {
                            ...
                            && j < currRun + 16; j++)
                        {
                            ...
                            while (!minHeap.isEmpty())
                            {
                                ...
                                int currRun2 = outputRecord.getFlag();  // The
                                                                        // run
                                                                        // of
                                                                        // the
                                                                        // record
                                ...
                                // Places the record that was just removed into
                                // the outputBuffer
                                ...
                                // respective block for that run are popped, add
                                // the next block of the run
                                if (recordsInHeap[currRun2] == 0)
                                {
                                    runHelperList2.get(currRun2)
                                        .incrementBlock();    // Increment the
                                                              // block that we
                                                              // are looking in
                                                              // the run

                                    // Now we have to grab the next block of
                                    // data from the run
                                    int currentBlock = runHelperList2
                                        .get(currRun2).getCurrentBlock();

                                    // The length of remaining data in the run
                                    // in bytes
                                    int lengthOfData =
                                        runHelperList2.get(currRun2).getLength()
                                            - (8192 * (currentBlock - 1));

                                    // Only grab the next block of data if there
                                    // is data left in the run
                                    if (lengthOfData > 0)
                                    {
                                        int startIndexBlock = runHelperList2
                                            .get(currRun2).getStartIndex()
                                            + (8192 * (currentBlock - 1));
                                        mergeFile.seek(startIndexBlock);  // Seek
                                                                          // to
                                                                          // the
                                                                          // beginning
                                                                          // of
                                        ...
                                        // If the run does not have at least a
                                        // block length of data then store up to
                                        ...
                                        // Else just read the whole block into
                                        // input buffer
                                        else
                                        {
                                            mergeFile.read(inputBuffer);
                                ...
                                // If the outputBuffer is full then dump it to
                                // merge file
                                if (outputIndex == BLOCK_SIZE)
                                {
                                     ...
                                     * block Record consoleRecord = new
                                     * Record(Arrays.copyOfRange(outputBuffer,
                                     * 0, 8)); if (recordsPrinted != 0 &&
                                     * recordsPrinted % 4 == 0) {
                                     * System.out.println();
                                     * System.out.print(consoleRecord); } else {
                                     * // If it's not the first record printed
                                     * don't add comma and // space if
                                     * (recordsPrinted == 0) {
                                     * System.out.print(consoleRecord); } else {
                ...
                else
                {
                    ...
                    System.out
                        .println("The runList size is: " + runList.size());
                    ...
                    for (int i = 0; i < runList.size(); i += 16)
                    {
                            ...
                            && j < currRun + 16; j++)
                        {
                            ...
                            while (!minHeap.isEmpty())
                            {
                                ...
                                if (recordsInHeap[currRun2] == 0)
                                {
                                    ...
                                    // Now we have to grab the next block of
                                    // data from the run
                                    int currentBlock = runHelperList
                                        .get(currRun2).getCurrentBlock();
                                    ...
                                    if (lengthOfData > 0)
                                    {
                                        int startIndexBlock = runHelperList
                                            .get(currRun2).getStartIndex()
                                            + (8192 * (currentBlock - 1));
                                        runFile.seek(startIndexBlock);  // Seek
                                                                        // to
                                                                        // the
                                                                        // beginning
                                                                        // of
                        ...
                        runList2.add(runCounter);      // Finished the "bigger"
                                                       // run
                    }
