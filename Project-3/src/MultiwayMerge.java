...
public class MultiwayMerge {
    ...
    public static void merge(
        ...
        RandomAccessFile runFile, String outputFileName) throws IOException {
        int total = 0;
        
        // The file we will be multi-way merging with
        RandomAccessFile mergeFile = new RandomAccessFile(outputFileName, "rw");
    	...
    	if (runList.size() <= 16) {
    	    mergeRuns(minHeap, runList, runFile,  0,  runList.size()-1, mergeFile, runHelperList);
    	}
    	// Then we grab the first 16 runs and repeat the process
            ...
            // Boolean to check if mergefile is sorted
            // If false at the end of the loop 
            // runFile is our output, otherwise merge is our output
            boolean mergeMoreSorted = false;    
                	    
    	    // The run list for the "bigger" runs, use when going from mergeFile to runFile        	    
    	    ...
    	    do {
    	        ...
    	        if (mergeMoreSorted) {
    	            ...
    	            runHelperList.clear();
    	            runFile.seek(0);
    	            System.out.println("The runList2 size: "+ runList2.size());
    	            System.out.println(runList2.get(1));
                    ...
                    for (int i = 0; i < runList2.size(); i += 16) {
                        ...
                        int currRun = i;   // Save the value of the beginning run we're looking at
                        int runCounter = 0;    // The number of records in each run
                        
                        // Loop through the next set of 16 runs or the remaining runs left
                        // For every run we grab the first block of data and repeat multiway merge process
                        for (int j = i; j < runList2.size() && j < currRun + 16; j++) {
                            
                            mergeFile.seek(runHelperList2.get(j).getStartIndex());    // Look to where the run begins in the runFile
                            ...
                            if (runHelperList2.get(j).getLength() < BLOCK_SIZE) {
                                mergeFile.read(inputBuffer, 0, runHelperList2.get(j).getLength());
                            }
                            ...
                            for (int k = 0; k < inputBuffer.length; k+=8) {
                                ...
                                Record temp = new Record(Arrays.copyOfRange(inputBuffer, k, k+8), j);
                                minHeap.insert(temp);
                                recordsInHeap[j]++;    // Increment the # of records in the heap for the respective run
                            }
                            ...
                            while (!minHeap.isEmpty()) {
                                ...
                                int currRun2 = outputRecord.getFlag();  // The run of the record popped out
                                ...
                                recordsInHeap[currRun2]--;    // Decrement the recordsInHeap array
                                ...
                                if (recordsInHeap[currRun2] == 0) {
                                    runHelperList2.get(currRun2).incrementBlock();    //Increment the block that we are looking in the run
                                    
                                    // Now we have to grab the next block of data from the run
                                    int currentBlock = runHelperList2.get(currRun2).getCurrentBlock();   
                                    
                                    // The length of remaining data in the run in bytes
                                    int lengthOfData = runHelperList2.get(currRun2).getLength() - (8192 * (currentBlock-1));
                                    
                                    // Only grab the next block of data if there is data left in the run
                                    if (lengthOfData > 0) {
                                        int startIndexBlock = runHelperList2.get(currRun2).getStartIndex() + (8192 * (currentBlock - 1));
                                        mergeFile.seek(startIndexBlock);  // Seek to the beginning of the next block of data
                                        
                                        // If the run does not have at least a block length of data then store up to the data that it has
                                        if (lengthOfData < BLOCK_SIZE) {
                                            mergeFile.read(inputBuffer, 0, lengthOfData);
                                        }
                                        // Else just read the whole block into input buffer
                                        else {
                                            mergeFile.read(inputBuffer);
                                        ...
                                        for (int k = 0; k < lengthOfData && k < inputBuffer.length; k+=8) {
                                            // Make the record have a flag for it's respective run
                                            Record temp = new Record(Arrays.copyOfRange(inputBuffer, k, k+8), currRun2);
                                            minHeap.insert(temp);
                                            recordsInHeap[currRun2]++;    // Increment the # of records in the heap for the respective run
                                        }
                                    }
                                ...
                                if (outputIndex == BLOCK_SIZE) {
                                    ...
                                    Record consoleRecord = new Record(Arrays.copyOfRange(outputBuffer, 0, 8));
                        ...
                        runList.add(runCounter);      // Finished the "bigger" run             
                    }
    	        ...
    	        // We are merging from runFile to mergeFile
    	            ...
    	            System.out.println("The runList size is: " + runList.size());
    	            ...
    	            for (int i = 0; i < runList.size(); i += 16) {
    	                ...
    	                // Loop through the next set of 16 runs or the remaining runs left
    	                // For every run we grab the first block of data and repeat multiway merge process
    	                for (int j = i; j < runList.size() && j < currRun + 16; j++) {
    	                    
    	                    runFile.seek(runHelperList.get(j).getStartIndex());    // Look to where the run begins in the runFile
    	                    
    	                    // If the run file does not have at least a block length of data then store up to the data that it has
    	                    if (runHelperList.get(j).getLength() < BLOCK_SIZE) {
    	                        runFile.read(inputBuffer, 0, runHelperList.get(j).getLength());
    	                    }
    	                    // Else just read the whole block into input buffer
    	                    else {
    	                    ...
    	                    while (!minHeap.isEmpty()) {
    	                        ...
    	                        // If all the records from the heap from that respective block for that run are popped, add the next block of the run
    	                        if (recordsInHeap[currRun2] == 0) {
    	                            runHelperList.get(currRun2).incrementBlock();    //Increment the block that we are looking in the run
    	                            
    	                            // Now we have to grab the next block of data from the run
    	                            int currentBlock = runHelperList.get(currRun2).getCurrentBlock();   
    	                            
    	                            // The length of remaining data in the run in bytes
    	                            int lengthOfData = runHelperList.get(currRun2).getLength() - (8192 * (currentBlock-1));
    	                            
    	                            // Only grab the next block of data if there is data left in the run
    	                            if (lengthOfData > 0) {
    	                                int startIndexBlock = runHelperList.get(currRun2).getStartIndex() + (8192 * (currentBlock - 1));
    	                ...
    	                runList2.add(runCounter);      // Finished the "bigger" run             
    	            }
    	            ...
    	            for (int i = 1; i < runList2.size(); i++) {         
    	                // We insert the rest of the runs to the runHelperList
    	                int startIndex = runHelperList2.get(i-1).getStartIndex() + runHelperList2.get(i-1).getLength();
    	                runHelperList2.add(new RunHelper(startIndex, runList2.get(i) * 8)); 
    	            }
    	    ...
    	    System.out.println("Final: " + runList.size());
    	    System.out.println("Final: " + runList2.size());
    ...
    /**
     * This is a helper function to merge runs and dump it to the mergeFile as one run
     * 
     * @param minHeap The heap that we will use for memory and sorting 
     * @param runList The list of runs from selection sort
     * @param runFile The runFile that stores all the runs
     * @param startRun The start of runs that we will be merging
     * @param endRun The end of runs that we will be merging 
     * @param mergeFile The mergeFile that we will be outputting to
     * @throws IOException 
     */
    private static void mergeRuns(Heap minHeap, ArrayList<Integer> runList,
        RandomAccessFile runFile, int startRun, int endRun,
        RandomAccessFile mergeFile, ArrayList<RunHelper> runHelperList) throws IOException {
        ...
        while (!minHeap.isEmpty()) {
            ...
            int currRun = outputRecord.getFlag();  // The run of the record popped out
            ...
            if (recordsInHeap[currRun] == 0) {
                ...
                if (lengthOfData > 0) {
                    ...
                    // Insert the records in the input buffer into the heap
                    for (int i = 0; i < lengthOfData && i < inputBuffer.length; i+=8) {
                        // Make the record have a flag for it's respective run
