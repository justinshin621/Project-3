...
public class MultiwayMerge {
     ...
     *            The String of the output file name
     * @throws IOException
     */
    public static void merge(
        ...
        if (runList.size() <= 16) {
            ...
            RandomAccessFile mergeFile = new RandomAccessFile(outputFileName,
                "rw");
            mergeRuns(minHeap, runList, runFile, 0, runList.size() - 1,
                mergeFile, runHelperList);
        ...
        // Then we grab the first 16 runs and repeat the process
            ...
            // The file we will be multi-way merging with
            RandomAccessFile mergeFile = new RandomAccessFile("mergefile.bin",
                "rw");
            ...
            do {
                ...
                if (mergeMoreSorted) {
                    ...
                    for (int i = 0; i < runList2.size(); i += 16) {
                        ...
                        for (int j = i; j < runList2.size() && j < currRun
                            + 16; j++) {

                            // Look to where run begins in the file
                            mergeFile.seek(runHelperList2.get(j)
                                .getStartIndex());

                            // If the run file does not have at least a block
                            // length of data then store up to the data that it
                            // has
                            if (runHelperList2.get(j)
                                .getLength() < BLOCK_SIZE) {
                                mergeFile.read(inputBuffer, 0, runHelperList2
                                    .get(j).getLength());
                            }
                            ...
                            for (int k = 0; k < inputBuffer.length; k += 8) {
                                ...
                                // respective run
                                Record temp = new Record(Arrays.copyOfRange(
                                    inputBuffer, k, k + 8), j);
                                minHeap.insert(temp);
                        ...
                        while (!minHeap.isEmpty()) {
                            ...
                            if (recordsInHeap[currRun2] == 0) {
                                // Increment the block in the run
                                runHelperList2.get(currRun2).incrementBlock();
                                ...
                                // bytes
                                int lengthOfData = runHelperList2.get(currRun2)
                                    .getLength() - (8192 * (currentBlock - 1));

                                // Only grab the next block of data if there is
                                // data left in the run
                                if (lengthOfData > 0) {
                                    int startIndexBlock = runHelperList2.get(
                                        currRun2).getStartIndex() + (8192
                                            * (currentBlock - 1));
                                    // Seek to the beginning of the block
                                    mergeFile.seek(startIndexBlock);

                                    // If the run does not have at least a block
                                    // length of data then store up to the data
                                    // that it has
                                    if (lengthOfData < BLOCK_SIZE) {
                                        mergeFile.read(inputBuffer, 0,
                                            lengthOfData);
                                    }
                                    ...
                                    for (int k = 0; k < lengthOfData
                                        ...
                                        // Make the record have a flag for it's
                                        // respective run
                                        Record temp = new Record(Arrays
                                            .copyOfRange(inputBuffer, k, k + 8),
                                            currRun2);
                                        minHeap.insert(temp);
                                        // Increment the # of records for that
                                        // run
                                        recordsInHeap[currRun2]++;
                            ...
                            if (outputIndex == BLOCK_SIZE) {
                                ...
                                if (runList.size() < 16) {
                                    ...
                                    // block
                                    Record consoleRecord = new Record(Arrays
                                        .copyOfRange(outputBuffer, 0, 8));

                                    if (recordsPrinted != 0 && recordsPrinted
                                        % 4 == 0) {
                                        System.out.println();
                    ...
                    for (int i = 1; i < runList.size(); i++) {
                        // We insert the rest of the runs to the runHelperList
                        int startIndex = runHelperList.get(i - 1)
                            .getStartIndex() + runHelperList.get(i - 1)
                                .getLength();
                        runHelperList.add(new RunHelper(startIndex, runList.get(
                            i) * 8));
                    }
                ...
                // We are merging from runFile to mergeFile
                    ...
                    for (int i = 0; i < runList.size(); i += 16) {
                        ...
                        // For every run we grab the first block of data and
                        // repeat multiway merge process
                        for (int j = i; j < runList.size() && j < currRun
                            + 16; j++) {

                            // Seek to the beginning of the run
                            runFile.seek(runHelperList.get(j).getStartIndex());

                            // If the run file does not have at least a block
                            // length of data then store up to the data that it
                            // has
                            if (runHelperList.get(j).getLength() < BLOCK_SIZE) {
                                runFile.read(inputBuffer, 0, runHelperList.get(
                                    j).getLength());
                            }
                            // Else just read the whole block into input buffer
                            else {
                        ...
                        while (!minHeap.isEmpty()) {
                            ...
                            if (recordsInHeap[currRun2] == 0) {
                                ...
                                // Increment the block that we are in the run
                                runHelperList.get(currRun2).incrementBlock();
                                ...
                                // The length of remaining data in the run in
                                // bytes
                                int lengthOfData = runHelperList.get(currRun2)
                                    .getLength() - (8192 * (currentBlock - 1));
                                ...
                                if (lengthOfData > 0) {
                                    ...
                                    // data
                                    int startIndexBlock = runHelperList.get(
                                        currRun2).getStartIndex() + (8192
                                            * (currentBlock - 1));
                                    ...
                                    for (int k = 0; k < lengthOfData
                                        ...
                                        Record temp = new Record(Arrays
                                            ...
                                            currRun2);
                                        minHeap.insert(temp);
                                        // Increment the # of records in the run
                                        recordsInHeap[currRun2]++;
                                    }
                                }
                            } // if recordsInHeap[currRun2] == 0 end
                    ...
                    for (int i = 1; i < runList2.size(); i++) {
                        // We insert the rest of the runs to the runHelperList
                        int startIndex = runHelperList2.get(i - 1)
                            .getStartIndex() + runHelperList2.get(i - 1)
                                .getLength();
                        runHelperList2.add(new RunHelper(startIndex, runList2
                            .get(i) * 8));
                    }
    ...
    private static void mergeRuns(
        ...
        for (int i = startRun; i <= endRun; i++) {
            ...
            for (int j = 0; j < inputBuffer.length; j += 8) {
                ...
                Record temp = new Record(Arrays.copyOfRange(inputBuffer, j, j
                    + 8), i);
                minHeap.insert(temp);
        ...
        while (!minHeap.isEmpty()) {
            ...
            if (recordsInHeap[currRun] == 0) {
                ...
                // run
                if (lengthOfData > 0) {
                    int startIndexBlock = runHelperList.get(currRun)
                        .getStartIndex() + (8192 * (currentBlock - 1));
                    ...
                    for (int i = 0; i < lengthOfData
                        ...
                        // Make the record have a flag for it's respective run
                        Record temp = new Record(Arrays.copyOfRange(inputBuffer,
                            i, i + 8), currRun);
                        minHeap.insert(temp);
            ...
            if (outputIndex == BLOCK_SIZE) {
                ...
                Record consoleRecord = new Record(Arrays.copyOfRange(
                    outputBuffer, 0, 8));
